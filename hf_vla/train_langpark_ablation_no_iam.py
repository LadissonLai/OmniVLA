"""
train_langpark_no_iam.py

Ablation #1 (baseline): NO MemoryEnhancementModule, NO InstructionAlignmentHead.

This is the LangPark training pipeline with both proposed innovations removed:
  - No MEM tokens are inserted into the LLM sequence.
  - No instruction-alignment auxiliary loss.

Implementation note: removing the MEM tokens reduces the model to the base
`ExpVLAForActionPrediction` forward (sequence layout without the 16 MEM tokens),
so we simply reuse that base class instead of `LangParkVLAForActionPrediction`.
Because the dec/act/EOS tail is unchanged and the hidden states are indexed from
the END of the sequence, the tail offsets are identical to the full model.

Sequence layout (no MEM):
  BOS | sys_prompt | instruct | p_front | front | p_rear | rear | p_left | left |
  p_right | right | p_hist | hist | p_slots | dec(1) | act(32) | EOS(1)
"""
...
class LangParkNoIAMConfig:
    ...
    visualize_dir:  str  = "vis/vis_langpark_ablation_no_iam_train"
...
@draccus.wrap()
def train_langpark_ablation_no_iam(cfg: LangParkNoIAMConfig):
    ...
    if distributed_state.is_main_process:
        ...
        wandb.init(
            ...
            name="langpark_ablation_no_iam_training",
            dir=cfg.wandb_dir,
        )
    ...
    # ── 2. External modules (no MEM module, no alignment head) ────────────────
    ...
    all_modules = (vla, past_traj_projector, action_head, decision_head)
        ...
        for batch_idx, batch in enumerate(progress):
            ...
            with ExitStack() as sync_ctx:
                ...
                with torch.autocast("cuda", dtype=torch.bfloat16):
                    # Forward (base ExpVLA forward — no MEM tokens)
                    outputs = vla(
                        batch,
                    ...
                    # ── dec / act hidden states ────────────────────────────────────
                    # Tail layout: ... p_slots | dec(1) | act(32) | EOS(1)
                    # Hidden at pos -(NUM_ACT+3) = p_slots[last]: predicts dec (shift)
...
if __name__ == "__main__":
    train_langpark_ablation_no_iam()
